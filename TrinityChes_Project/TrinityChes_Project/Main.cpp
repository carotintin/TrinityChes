...
//●真・トリニティチェックメイト条件：
// 
//		・自分の駒の本来の行動範囲に相手の駒を３個入れたらトリニティ
//      ・いずれかの相手の駒移動可能範囲が自分の移動可能範囲と全て重複していたらトリニティ
//
//			
//●真・自爆メイト条件：
// 
//		・動く動けないに関わらず相手の駒の行動範囲に入ったら自爆
//      ・自分の駒を動かした時に相手の駒の行動範囲に自分の駒が２体いると自爆(本当はそうだけどやらない
//
...
// 
//		・OK  同じ駒が連続で動かせてるのを直す
//		・OK  動かせない駒の移動範囲でチェックメイト判定をさせないようにする
//		・OK  チェックメイト判定のバグ直し
//		・駒の配置フェーズを作る
//		・リスタート＆ゲーム終了表示
//		・プレイヤーターンの文字アニメーション
...
// 
//●備考
//		・自分の移動範囲に二つ以上の駒でチェックメイトは確かにそうだけど、
// 		　　置いた時点で探索範囲を消すから発生してない。
//				自分の未来の移動範囲に三つの駒でチェックメイトなんじゃないか
//		
//
